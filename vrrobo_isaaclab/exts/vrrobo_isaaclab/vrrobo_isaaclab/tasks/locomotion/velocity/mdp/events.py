...
def reset_robot_with_cones(
    ...
    assert len(x_range_list) == len(y_range_list) == len(z_range_list) == len(yaw_range_list), "The length of x, y, z, and yaw ranges should be the same."
    ...
    for i in range(len(env_ids)):
        ...
        for j in range(4):
            ...
            if j==0:
                ...
                rand_samples_robot = math_utils.sample_uniform(ranges[:, 0], ranges[:, 1], (4), device=asset_robot.device)
...
def reset_root_state_uniform_custom(
    ...
    * It samples the root velocity from the given ranges and sets them into the physics simulation.
    ...
    yaw_range_list = pose_range.get("yaw")
    z_range_list = pose_range.get("z")
    assert len(x_range_list) == len(y_range_list) == len(yaw_range_list) == len(pose_range_prob), "The length of x, y, and yaw ranges should be the same."
