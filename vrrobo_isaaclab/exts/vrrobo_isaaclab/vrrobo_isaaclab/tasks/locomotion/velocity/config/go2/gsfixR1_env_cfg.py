...
from vrrobo_isaaclab.assets import GALAXEA_R1_BASE_CFG  # isort: skip
...
class MySceneCfg(InteractiveSceneCfg):
    ...
    #     offset=TiledCameraCfg.OffsetCfg(pos=[0.332+0.01, 0.0+0.015, 0.0578+0.095], rot=[0.9799, 0.0000, 0.1994, 0.0000], convention="world"),
    #     data_types=["rgb","instance_id_segmentation_fast"],
    #     # data_types=["instance_id_segmentation_fast"],
    #     spawn=sim_utils.PinholeCameraCfg(
    #         focal_length=24.0, horizontal_aperture=23.9832939727*2, vertical_aperture=13.5192958675*2, clipping_range=(0.01, 100.0)
    #     ),
    #     width=320,
    #     height=180,
    ...
    object: RigidObjectCollectionCfg = RigidObjectCollectionCfg(
        rigid_objects={
            ...
            "wall_left": RigidObjectCfg(
                ...
                init_state=RigidObjectCfg.InitialStateCfg(pos=(2.0-0.7, 0.0+1.9, 0.5), rot=(1.0, 0.0, 0.0, 0.0)),
            ),
            "wall_right": RigidObjectCfg(
                ...
                init_state=RigidObjectCfg.InitialStateCfg(pos=(2.0-0.7, 0.0-3.5, 0.5), rot=(1.0, 0.0, 0.0, 0.0)),
            ),
            ...
            "wall_behind": RigidObjectCfg(
                ...
                init_state=RigidObjectCfg.InitialStateCfg(pos=(2.0-2.6, 0.0-0.8, 0.5), rot=(1.0, 0.0, 0.0, 0.0)),
            ),
        }
    )
...
class ActionsCfg:
    ...
    joint_pos = mdp.VelocityCommandActionCfg(asset_name="robot", joint_names=['torso_joint1', 'torso_joint2', 'torso_joint3', 'torso_joint4', 'left_arm_joint1', 'right_arm_joint1', 'left_arm_joint2', 'right_arm_joint2', 'left_arm_joint3', 'right_arm_joint3', 'left_arm_joint4', 'right_arm_joint4', 'left_arm_joint5', 'right_arm_joint5', 'left_arm_joint6', 'right_arm_joint6', 'left_gripper_finger_joint1', 'left_gripper_finger_joint2', 'right_gripper_finger_joint1', 'right_gripper_finger_joint2'], scale=0.25, use_default_offset=True, velocity_range=[0.5, 0.5, 1.0], 
        policy_dir="/home/zhust/codes/go2_isaaclab/2024-12-29_23-55-29/model_158000.pt", 
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        gs_image = ObsTerm(
            ...
            params={"camera_pos": [0.332+0.01, 0.0+0.015, 0.3578+0.095], "camera_rot":[0., 23., 0.], "asset_offset_pos": ASSET_OFFSET, "asset_offset_rot":[1.0, 0.0, 0.0, 0.0]},
            noise=None,
        )
...
class EventCfg:
    ...
    reset_base = EventTerm(
        ...
        mode="reset",
        params={
            "pose_range": {"x": (0.2, 0.25), "y": (-1.5, 0.5), "z": (0.05, 0.05), "yaw": (-1.0, 1.0)},
    ...
    # reset_robot_with_cones = EventTerm(
    #     func=mdp.reset_robot_with_cones_default,
    #     mode="reset",
    #     params={
    #         # "pose_range": {"x": [(0.1, 3.0), (0.1, 0.8), (1.0, 1.8), (2.0, 3.0), (2.15, 3.0)], 
    #         #             "y": [(0.2, 1.3), (-3.0, -0.2), (-3.0, -1.2), (-3.0, -1.9), (-1.05, -0.2)], 
...
class RewardsCfg:
    ...
    # track_lin_vel_xy_exp_command = RewTerm(func=mdp.track_lin_vel_xy_exp_command, weight=0.1, params={"std": math.sqrt(0.25)})
    # track_ang_vel_z_exp_command = RewTerm(func=mdp.track_ang_vel_z_exp_command, weight=0.1, params={"std": math.sqrt(0.25)})
    action_l2 = RewTerm(func=mdp.action_l2, weight=-0.0004)
...
class TerminationsCfg:
    ...
    # get_goal = DoneTerm(func=mdp.get_goal, params={"base_height": BASE_HEIGHT, "command_name": "rgb_command", "threshold": 0.25}, time_out=False)
...
@configclass
class GalaxeaR1GSBaseEnvCfg(ManagerBasedRLEnvCfg):
...
@configclass
class GalaxeaR1GSFixEnvCfg(GalaxeaR1GSBaseEnvCfg):
    def __post_init__(self):
        # post init of parent
        super().__post_init__()
        
        self.scene.num_envs = 48
        self.scene.robot = GALAXEA_R1_BASE_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
...
@configclass
class GalaxeaR1GSFixEnvCfg_PLAY(GalaxeaR1GSFixEnvCfg):
    def __post_init__(self):
        # post init of parent
        super().__post_init__()
