...
def main():
    ...
    camera_position = env.env.scene.env_origins[camera_follow_id, :3].cpu().numpy() + [1.5,-0.8,0] - camera_direction
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            actions = policy(obs)
            # env stepping
            time_step=env.env.episode_length_buf.clone()
            # print(env.env.episode_length_buf)
            ...
            if dones.any():
                ...
                success_list.append(~extras["time_outs"])
        ...
        if MOVE_CAMERA:
            ...
            env.sim.set_camera_view(camera_position, camera_position+camera_direction)
    # Calculate average time step and success rate
    avg_time_step = torch.mean(torch.stack(time_step_list).float())
    avg_success_rate = torch.mean(torch.cat(success_list).float())
    print(f"Average Time Step: {avg_time_step*0.2}")
