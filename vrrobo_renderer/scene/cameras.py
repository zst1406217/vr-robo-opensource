...
from utils.graphics_utils import fov2focal, getProjectionMatrix, getProjectionMatrixCenterShift, getWorld2View2
...
class Camera(nn.Module):
    ...
    def get_inv_k(self, scale=1.0):
        K_T = torch.tensor(
            [[scale / self.Fx, 0, -self.Cx / self.Fx], [0, scale / self.Fy, -self.Cy / self.Fy], [0, 0, 1]]
        ).cuda()
        return K_T
...
def sample_cam(cam_l: Camera, cam_r: Camera):
    ...
    cam.projection_matrix = (
        getProjectionMatrix(znear=cam.znear, zfar=cam.zfar, fovX=cam.FoVx, fovY=cam.FoVy).transpose(0, 1).cuda()
    )
